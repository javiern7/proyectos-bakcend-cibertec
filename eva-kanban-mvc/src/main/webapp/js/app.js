document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderActionsHtml(id, status, canAdvance, isAdmin) {
        if (isAdmin) {
            ...
            const nextForm = next ? `
      <form action="${window.ctx}/task/${id}/status" method="post" class="d-inline">
      ...
      <form action="${window.ctx}/task/${id}/delete" method="post" class="d-inline"
        ...
        // --- USER ---
        // canAdvance = (soy dueño && status !== 'DONE')
        if (status === 'DONE' || !canAdvance) {
            // Todo deshabilitado si está DONE o no es dueño
            return `
      ...
      <button class="btn btn-sm btn-outline-danger" disabled data-bs-toggle="tooltip"
              title="${status === 'DONE' ? 'Tarea finalizada' : 'Solo el dueño puede eliminar'}">Eliminar</button>`;
        }
        // Dueño y NO DONE: Editar / Cambiar estado / Eliminar habilitados
